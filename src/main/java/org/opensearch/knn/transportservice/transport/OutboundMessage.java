...
abstract class OutboundMessage extends NetworkMessage {
    ...
    static class Response extends OutboundMessage {
        ...
        private static byte setStatus(boolean compress, boolean isHandshake, Writeable message) {
            byte status = 0;
            status = org.opensearch.knn.transportservice.transport.TransportStatus.setResponse(status);
            if (message instanceof RemoteTransportException) {
                status = org.opensearch.knn.transportservice.transport.TransportStatus.setError(status);
            }
            if (compress) {
                status = org.opensearch.knn.transportservice.transport.TransportStatus.setCompress(status);
            }
            if (isHandshake) {
                status = org.opensearch.knn.transportservice.transport.TransportStatus.setHandshake(status);
            }
